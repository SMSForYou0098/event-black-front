import { FC, Fragment, memo, useState } from "react";

// Component
import SectionSlider from "../slider/SectionSlider";
// import ContinueWatchCard from "../cards/ContinueWatchCard";

// Function
import { generateImgPath } from "../../StaticData/data";
import ProductCard from "../cards/ProductCard";

const HomeArtists = memo(({ type }) => {
  const [title] = useState("Artists");
  const [watching, setWatching] = useState(
    Array.from({ length: 8 }, (_, i) => ({
      image: generateImgPath(`/assets/artists/${String(i + 1).padStart(2, "0")}.webp`),
      value: Math.floor(Math.random() * 100), // Optional: random progress for demo
    }))
  );


  return (
    <Fragment>
      <SectionSlider
        title={title}
        list={watching}
        className="continue-watching-block"
        slidesPerView={6}
        autoplay={{ delay: 3000, disableOnInteraction: false }}
        loop={watching.length > 5}
      >
        {(data) => (
          <ProductCard
            thumbnail={data.image}
            imageOnly={true}

          />
        )}
      </SectionSlider>
    </Fragment>
  );
});

HomeArtists.displayName = "HomeArtists";
export default HomeArtists;
