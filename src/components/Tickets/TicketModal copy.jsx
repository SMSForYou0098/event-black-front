...
// API fetch functions for TanStack Query
...
const TicketModal = (props) => {
    ...
    // TanStack Query for fetching layout
    const { data: savedLayout = {} } = useQuery({
        queryKey: ['layout', eventId],
        queryFn: () => fetchLayout(eventId),
        enabled: !!show && !!isIdCard && !!bgRequired && !!eventId,
        staleTime: 1000 * 60 * 10, // 10 minutes
        retry: 1,
    });

    // TanStack Query for fetching user photo
    const { data: userPhoto } = useQuery({
        queryKey: ['user-photo', ticketData?.Photo],
        queryFn: () => fetchImage(ticketData?.Photo),
        enabled: !!show && !!isIdCard && !!ticketData?.Photo,
        staleTime: 1000 * 60 * 30, // 30 minutes
        retry: 1,
    });

    // TanStack Query for fetching ID card background
    const { data: idCardBg } = useQuery({
        queryKey: ['id-card-bg', card_url],
        queryFn: () => fetchImage(card_url),
        enabled: !!show && !!isIdCard && !!bgRequired && !!card_url,
        staleTime: 1000 * 60 * 30, // 30 minutes
        retry: 1,
    });
