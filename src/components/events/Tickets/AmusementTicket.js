...
const AmusementTicket = (props) => {
    const { showDetails, ticketName,title, userName, number, address, ticketBG, date, time, photo, OrderId, showPrintButton } = props
      ...
      const drawCanvas = async () => {
        ...
        if (imageUrl) {
          try {
            ...
            if (showDetails) {
              // centerText(`${title}` || 'Event Name', 16, 'Arial', canvas, 55);
              centerText(`${ticketName}` || 'Ticket Name', 16, 'Arial', canvas, 215);
              centerText(`${userName}` || 'User Name', 16, 'Arial', canvas, 235);
              centerText(`${number}` || 'User Number', 16, 'Arial', canvas, 255);
...
export default AmusementTicket
