...
// react-bootstrap
import { Button, Nav, Collapse, Navbar, Offcanvas, Container, Dropdown, Image } from "react-bootstrap";
...
import { getActiveMenu, MenuItem } from "@/services/home";
...
const HeaderDefault = memo(() => {
  ...
  const { UserData, createSlug, isLoggedIn, systemSetting } = useMyContext();
  const [show1, setShow1] = useState(false);
  const [show, setShow] = useState(false);
  const dispatch = useDispatch();
    const [isOpen, setIsOpen] = useState(false);

 const handleToggle = (nextShow:any, event:any) => {
  ...
  const { data: menu = [], isLoading, isError, error } = useQuery<MenuItem[], Error>({
    queryKey: ["activeMenu"],
  ...
  ></button>
                  ...
                  <ul className="navbar-nav iq-nav-menu list-unstyled" id="header-menu">
                    {menu.map((item) => {
                    let href = item.external_url || (item.page ? `/${createSlug(item.page.title)}` : "#");
              ...
              <div className="right-panel">
                ...
                >
                  <ul className="navbar-nav align-items-center ms-auto mb-2 mb-xl-0">
                    ...
                    {
                      isLoggedIn ? <Dropdown as="li" className="nav-item">
                      <Dropdown.Toggle
                        as={CustomToggle}
                        href="#"
